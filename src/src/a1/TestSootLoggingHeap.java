		...
		if(!b.getMethod().getDeclaringClass().getName().equals("a1.Log"))
		{
		    ...
		    while(it.hasNext()){
		    	...
		    	if (stmt.containsFieldRef()) {
		    		...
		    		boolean static_status = stmt.getFieldRef().getField().isStatic();//checking if the field is static or not
		    		String name = stmt.getFieldRef().getField().getSignature().toString();//setting the name to the signature from the getfieldref
		    		boolean write_status = false;//initializing the write status as false
		    		if(stmt.getDefBoxes().toString().contains("($")) {//checking if the getDefBox string contains a $ or not 
		    			write_status = false;//if so write status is true
		    		}else {
		    			write_status = true;//else write status is false
		    		}
		    		a1.Log.logFieldAcc(Thread.currentThread(), name, static_status, write_status);//calling the LogFieldAcc from the Log class
